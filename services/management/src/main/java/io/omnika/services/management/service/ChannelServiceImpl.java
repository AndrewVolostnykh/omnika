...
class ChannelServiceImpl implements ChannelService {
    ...
    public ChannelDto create(UUID tenantId, ChannelDto channelDto) {
        ...
        // it should be supplier but not stream bridge
        streamBridge.send("newTelegramChannel-in-0", channelDto);
    ...
    public Consumer<ChannelType> getAllChannels() {
        ...
        return channelType -> {
            ...
            streamBridge.send("allChannels-in-0", hibernateUtils.doInNewTransaction(() -> listAllByType(channelType)));
        };
    }
